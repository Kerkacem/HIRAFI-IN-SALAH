import React from 'react';
import { ServiceCategory, Artisan, Review } from './types';

export const WHATSAPP_NUMBER = "213661379535";

export const CATEGORIES: ServiceCategory[] = [
    { id: "electricity", name: "كهرباء" },
    { id: "plumbing", name: "سباكة / ماء" },
    { id: "construction", name: "بناء / ترميم" },
    { id: "mechanic", name: "ميكانيك" },
    { id: "ac", name: "تكييف / سخان" },
    { id: "other", name: "أخرى" },
];

export const DEMO_ARTISANS: Artisan[] = [
    { id: "1", name: "محمد بن علي", skill: "كهرباء", yearsOfExperience: 5, neighborhood: "حي الزاوية", phone: "0661000000", verified: true, bio: "كهربائي متخصص في تمديدات المنازل الجديدة وصيانة الأعطال الطارئة. ملتزم بالمواعيد والجودة." },
    { id: "2", name: "أحمد قادري", skill: "سباكة", yearsOfExperience: 7, neighborhood: "حي الرملية", phone: "0662000000", verified: true, bio: "سباك بخبرة طويلة في جميع أنواع السباكة، تركيب وإصلاح. خدمة سريعة ومضمونة." },
    { id: "3", name: "عبد القادر", skill: "بناء", yearsOfExperience: 10, neighborhood: "وسط المدينة", phone: "0663000000", bio: "أقوم بجميع أعمال البناء والترميم والتشطيبات. الدقة في العمل هي شعاري." },
];

export const DEMO_REVIEWS: Review[] = [
    { id: 'r1', artisanId: '1', clientName: 'علي', rating: 5, comment: 'خدمة ممتازة وسريعة، أنصح به بشدة.', date: '2024-05-20T10:00:00Z' },
    { id: 'r2', artisanId: '2', clientName: 'فاطمة', rating: 4, comment: 'عمل جيد ومتقن، لكن تأخر قليلاً عن الموعد.', date: '2024-05-18T14:30:00Z' },
    { id: 'r3', artisanId: '1', clientName: 'سارة', rating: 5, comment: 'محترف جداً ويعرف عمله جيداً.', date: '2024-05-15T09:00:00Z' },
    { id: 'r4', artisanId: '3', clientName: 'يوسف', rating: 5, comment: 'أفضل بنّاي في المنطقة، عمل نظيف ومثالي.', date: '2024-05-21T11:00:00Z' },
    { id: 'r5', artisanId: '2', clientName: 'خالد', rating: 5, comment: 'حل المشكلة بسرعة وكفاءة.', date: '2024-05-22T16:00:00Z' },
];


export const IconHome = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg>
);

export const IconList = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><line x1="8" x2="21" y1="6" y2="6"></line><line x1="8" x2="21" y1="12" y2="12"></line><line x1="8" x2="21" y1="18" y2="18"></line><line x1="3" x2="3.01" y1="6" y2="6"></line><line x1="3" x2="3.01" y1="12" y2="12"></line><line x1="3" x2="3.01" y1="18" y2="18"></line></svg>
);

export const IconBuild = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"></path></svg>
);

export const IconPerson = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
);

export const IconShoppingCart = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><circle cx="8" cy="21" r="1"></circle><circle cx="19" cy="21" r="1"></circle><path d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.16"></path></svg>
);

export const IconPhone = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
);

export const IconWhatsApp = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" {...props}>
        <path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38c1.45.79 3.08 1.21 4.79 1.21 5.46 0 9.91-4.45 9.91-9.91S17.5 2 12.04 2zM12.04 20.12c-1.48 0-2.92-.4-4.2-1.15l-.3-.18-3.12.82.83-3.04-.2-.31c-.82-1.31-1.26-2.83-1.26-4.38 0-4.54 3.7-8.24 8.24-8.24 4.54 0 8.24 3.7 8.24 8.24s-3.7 8.24-8.24 8.24zm4.52-6.14c-.25-.12-1.47-.72-1.7-.8s-.39-.12-.56.12c-.17.25-.64.8-.79.97s-.3.17-.56 0c-.25-.12-1.07-.38-2.04-1.26s-1.47-1.8-1.64-2.1c-.17-.3-.02-.46.1-.61s.25-.3.38-.45c.12-.12.17-.2.25-.33s.12-.25 0-.38c-.12-.12-.56-1.34-.76-1.84s-.4-.42-.56-.42c-.16 0-.34-.02-.51-.02s-.44.06-.68.33c-.24.27-.92.9-1.12 2.2s.25 2.55.28 2.72c.04.17 1.14 1.75 2.78 2.45s1.1.65 1.48.52c.38-.13 1.18-.48 1.34-.94s.17-.85.12-1.02c-.04-.17-.16-.25-.4-.37z"/>
    </svg>
);

export const IconStar = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
);

export const IconCheckCircle = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
);

export const IconChevronRight = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="m9 18 6-6-6-6"></path></svg>
);

export const IconArrowLeft = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
);